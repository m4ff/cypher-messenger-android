...
public class DBManagerAndroidImpl implements DBManager {
    ...
    public CypherMessage getLastMessage(CypherUser contact) {
        SQLiteDatabase db = openHelper.getReadableDatabase();
        String[] columns = new String[]{DBHelper.COLUMN_MESSAGE_ID, DBHelper.COLUMN_MESSAGE_IS_SENDER, DBHelper.COLUMN_MESSAGE_TEXT, DBHelper.COLUMN_MESSAGE_SENT, DBHelper.COLUMN_MESSAGE_DATE_TIME};
        Cursor c = db.query(DBHelper.TABLE_MESSAGES, columns, DBHelper.COLUMN_MESSAGE_CONTACT_ID + " = ?", new String[]{contact.getUserID() + ""}, "", "", DBHelper.COLUMN_MESSAGE_DATE_TIME + " DESC", "1");
        CypherMessage m = null;
        if(c.moveToNext()) {
            m = new CypherMessage(c.getInt(0), c.getString(2), c.getLong(4), c.getInt(1) != 0, contact.getUserID(), c.getInt(3) != 0);
        }
        return m;
    }
